...
| Command                | Application                        |
| ---------------------- | ---------------------------------- |
| npm run dev:storefront | Next.js App Router, port 3100      |
| npm run dev:merchant   | React/Vite/Router, port 4200       |
| npm run dev:admin      | React/Vite/Router, port 4300       |
| npm run dev:api        | NestJS, port 3101; GET /api/health |
...
On Windows, if the Playwright browser download is unavailable, use installed Edge:
...
Local ports are 3100 (storefront), 3101 (API), 4200 (merchant) and 4300 (admin).
Ports 3000/3001 fall inside this machine's Windows reserved range (2906–3005).
If Windows reports EACCES on a port, inspect `netsh interface ipv4 show excludedportrange protocol=tcp` before choosing another port.

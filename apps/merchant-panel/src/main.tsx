...
document.title = locales[locale].messages.merchant;
function Home() {
  const { messages } = useLocale();
  return (
    <main className="mx-auto max-w-4xl px-6 py-16">
      <h1 className="text-3xl font-bold">{messages.merchant}</h1>
      <p className="mt-4">{messages.merchantDescription}</p>
    ...
    <>
      <header className="flex flex-wrap items-center justify-between gap-4 border-b px-6 py-4">
        <nav aria-label={messages.navigation}>
          <Link to="/">{messages.merchant}</Link>
        </nav>
        <LanguageSwitcher />
      </header>
      <Routes>
